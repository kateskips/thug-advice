document.addEventListener('DOMContentLoaded', () => {
	...
	sorting.addEventListener('click', (event) => {
		...
		fetch('http://localhost:3000/advices').then((resp) => resp.json()).then((quotes) => {
			...
			for (const quote of quotes) {
				sortedCollection.innerHTML += `<div class="sorting-card">
			...
			.then((arrayOfTypes) => {
				...
				for (const type of arrayOfTypes) {
					...
					b.addEventListener('click', (e) => {
						e.preventDefault();
						typeCollection.innerHTML = `<div class="type-card">
	...
	if (adviceForm != null) {
		...
		fetch('http://localhost:3000/users').then((resp) => resp.json()).then((arrayOfTypes) => {
			for (const type of arrayOfTypes) {
